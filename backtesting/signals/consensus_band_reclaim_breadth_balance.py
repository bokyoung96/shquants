...
class ConsensusBandReclaimBreadthBalanceSignalProducer:
    revision_lookback: int = 20
    trend_lookback: int = 60
    pullback_lookback: int = 8
    max_hold_days: int = 10
    revision_threshold: float = 0.0105
    volume_threshold: float = 0.90
    reclaim_buffer: float = -0.0020
    ...
    def build(self, market: MarketData) -> SignalBundle:
        ...
        revision_gate = op_q1_rev.gt(self.revision_threshold) & op_y1_rev.gt(-0.006)

        prior_20d = close.shift(1).divide(close.shift(21)).sub(1.0)
        trend_ok = close.gt(sma60 * 0.972) & sma20.gt(sma60 * 0.988) & prior_20d.ge(-0.09) & prior_20d.le(0.32)

        pullback_depth = close.shift(1).divide(high.shift(1).rolling(self.pullback_lookback, min_periods=4).max()).sub(1.0)
        pullback_ok = pullback_depth.le(-0.011) & pullback_depth.ge(-0.13)
        ...
        contraction_ok = contraction_ratio.le(0.82)
        ...
        stable_regime = market_breadth.shift(1).ge(0.46) & breadth_ma5.shift(1).ge(0.45)
        recovery_regime = market_breadth.shift(1).ge(0.41) & breadth_ma5.shift(1).ge(0.41) & breadth_3d_change.shift(1).ge(0.025)
        expansion_regime = market_breadth.shift(1).ge(0.39) & breadth_ma10.shift(1).ge(0.40) & breadth_10d_change.shift(1).ge(0.045)
        ...
        close_strength = close.ge(open_ * 0.999) & close.ge((high + low) / 2.0)
        ...
        trailing_support = pd.concat([
            sma10 * 0.987,
            sma20 * 0.982,
            low.shift(1).rolling(4, min_periods=2).min() * 0.987,
        ], axis=0).groupby(level=0).max()
        breadth_exit = market_breadth.lt(0.385) | breadth_3d_change.lt(-0.095) | breadth_10d_change.lt(-0.11)
        ...
        alpha = (
            ...
            - 0.055 * pullback_depth.abs()
            + 0.08 * volume_ratio
            + 0.08 * close.divide(sma10.replace(0.0, pd.NA)).sub(1.0)
            + 0.08 * close.divide(sma20.replace(0.0, pd.NA)).sub(1.0)
            + 0.07 * contraction_ratio.rsub(1.0)
            + 0.08 * pd.DataFrame({col: market_breadth for col in close.columns}, index=close.index)
            + 0.07 * pd.DataFrame({col: breadth_3d_change.fillna(0.0) for col in close.columns}, index=close.index)
            + 0.04 * pd.DataFrame({col: breadth_10d_change.fillna(0.0) for col in close.columns}, index=close.index)
