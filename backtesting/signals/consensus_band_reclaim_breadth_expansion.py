...
class ConsensusBandReclaimBreadthExpansionSignalProducer:
    revision_lookback: int = 20
    trend_lookback: int = 60
    pullback_lookback: int = 8
    max_hold_days: int = 8
    revision_threshold: float = 0.0095
    volume_threshold: float = 0.78
    reclaim_buffer: float = -0.0025
    ...
    def build(self, market: MarketData) -> SignalBundle:
        ...
        revision_gate = op_q1_rev.gt(self.revision_threshold) & op_y1_rev.gt(-0.009)

        prior_20d = close.shift(1).divide(close.shift(21)).sub(1.0)
        trend_ok = close.gt(sma60 * 0.968) & sma20.gt(sma60 * 0.985) & prior_20d.ge(-0.11) & prior_20d.le(0.34)

        pullback_depth = close.shift(1).divide(high.shift(1).rolling(self.pullback_lookback, min_periods=4).max()).sub(1.0)
        pullback_ok = pullback_depth.le(-0.008) & pullback_depth.ge(-0.145)
        support_ok = low.shift(1).rolling(3, min_periods=2).min().gt(sma60.shift(1) * 0.938)
        ...
        contraction_ok = contraction_ratio.le(0.87)
        ...
        breadth_ma10 = market_breadth.rolling(10, min_periods=5).mean()
        breadth_3d_change = market_breadth.diff(3)
        breadth_10d_change = market_breadth.diff(10)

        stable_regime = market_breadth.shift(1).ge(0.45) & breadth_ma5.shift(1).ge(0.44)
        recovery_regime = market_breadth.shift(1).ge(0.40) & breadth_ma5.shift(1).ge(0.40) & breadth_3d_change.shift(1).ge(0.02)
        expansion_regime = market_breadth.shift(1).ge(0.37) & breadth_ma10.shift(1).ge(0.38) & breadth_10d_change.shift(1).ge(0.05)
        breadth_regime_ok = stable_regime | recovery_regime | expansion_regime
        ...
        reclaim = close.gt(reclaim_ref * (1.0 + self.reclaim_buffer)) & open_.le(reclaim_ref * 1.022)
        close_strength = close.ge(open_ * 0.996) & close.ge((high + low) / 2.0)
        medium_trend_ok = close.ge(sma120 * 0.965)
        ...
        trailing_support = pd.concat([
            sma10 * 0.985,
            sma20 * 0.979,
            low.shift(1).rolling(4, min_periods=2).min() * 0.984,
        ], axis=0).groupby(level=0).max()
        breadth_exit = market_breadth.lt(0.36) | breadth_3d_change.lt(-0.11) | breadth_10d_change.lt(-0.13)
        ...
        alpha = (
            0.36 * op_q1_rev
            + 0.14 * op_y1_rev
            + 0.10 * close.divide(reclaim_ref.replace(0.0, pd.NA)).sub(1.0)
            - 0.05 * pullback_depth.abs()
            + 0.08 * volume_ratio
            + 0.07 * close.divide(sma10.replace(0.0, pd.NA)).sub(1.0)
            + 0.07 * close.divide(sma20.replace(0.0, pd.NA)).sub(1.0)
            + 0.07 * contraction_ratio.rsub(1.0)
            + 0.08 * pd.DataFrame({col: market_breadth for col in close.columns}, index=close.index)
            + 0.08 * pd.DataFrame({col: breadth_3d_change.fillna(0.0) for col in close.columns}, index=close.index)
            + 0.06 * pd.DataFrame({col: breadth_10d_change.fillna(0.0) for col in close.columns}, index=close.index)
            + 0.07 * close.divide(sma120.replace(0.0, pd.NA)).sub(1.0)
        ...
        return SignalBundle(
            ...
            meta={
                ...
                'breadth_ma10': breadth_ma10,
                'breadth_3d_change': breadth_3d_change,
                'breadth_10d_change': breadth_10d_change,
