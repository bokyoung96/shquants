...
class ConsensusBandReclaimBreadthReaccelSignalProducer:
    ...
    def build(self, market: MarketData) -> SignalBundle:
        ...
        trend_ok = close.gt(sma60 * 0.972) & sma20.gt(sma60 * 0.988) & prior_20d.ge(-0.09) & prior_20d.le(0.34)

        pullback_depth = close.shift(1).divide(high.shift(1).rolling(self.pullback_lookback, min_periods=4).max()).sub(1.0)
        pullback_ok = pullback_depth.le(-0.010) & pullback_depth.ge(-0.14)
        ...
        contraction_ok = contraction_ratio.le(0.88)
        ...
        breadth_delta3 = market_breadth.diff(3)
        breadth_reaccel = (
            (market_breadth.shift(1).ge(0.42) & breadth_ma5.shift(1).ge(0.43) & breadth_delta3.shift(1).ge(-0.01))
            | (market_breadth.shift(1).ge(0.46) & breadth_ma5.shift(1).ge(0.45) & breadth_delta3.shift(1).ge(-0.03))
        )
        breadth_frame = pd.DataFrame({col: breadth_reaccel for col in close.columns}, index=close.index).fillna(False)
        ...
        close_strength = close.ge(open_ * 0.997) & close.ge((high + low) / 2.0) & close.ge(sma5 * 0.996)
        ...
        breadth_exit = market_breadth.lt(0.38) | breadth_delta3.lt(-0.10)
        ...
        alpha = (
            ...
            + 0.10 * pd.DataFrame({col: market_breadth for col in close.columns}, index=close.index)
            + 0.08 * pd.DataFrame({col: breadth_delta3 for col in close.columns}, index=close.index)
            + 0.10 * close.divide(sma120.replace(0.0, pd.NA)).sub(1.0)
