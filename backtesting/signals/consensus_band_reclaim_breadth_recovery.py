...
class ConsensusBandReclaimBreadthRecoverySignalProducer:
    ...
    def build(self, market: MarketData) -> SignalBundle:
        ...
        trend_ok = close.gt(sma60 * 0.975) & sma20.gt(sma60 * 0.99) & prior_20d.ge(-0.08) & prior_20d.le(0.31)

        pullback_depth = close.shift(1).divide(high.shift(1).rolling(self.pullback_lookback, min_periods=4).max()).sub(1.0)
        pullback_ok = pullback_depth.le(-0.014) & pullback_depth.ge(-0.125)
        support_ok = low.shift(1).rolling(3, min_periods=2).min().gt(sma60.shift(1) * 0.943)
        ...
        stable_regime = market_breadth.shift(1).ge(0.47) & breadth_ma5.shift(1).ge(0.46)
        recovery_regime = market_breadth.shift(1).ge(0.42) & breadth_ma5.shift(1).ge(0.42) & breadth_3d_change.shift(1).ge(0.03)
        breadth_regime_ok = stable_regime | recovery_regime
        ...
        medium_trend_ok = close.ge(sma120 * 0.975)
        ...
        breadth_exit = market_breadth.lt(0.39) | breadth_3d_change.lt(-0.10)
        ...
        alpha = (
            ...
            + 0.09 * pd.DataFrame({col: market_breadth for col in close.columns}, index=close.index)
