...
class ConsensusBandReclaimBreadthRegimeSignalProducer:
    ...
    def build(self, market: MarketData) -> SignalBundle:
        ...
        revision_gate = op_q1_rev.gt(self.revision_threshold) & op_y1_rev.gt(-0.003)

        prior_20d = close.shift(1).divide(close.shift(21)).sub(1.0)
        trend_ok = close.gt(sma60 * 0.975) & sma20.gt(sma60 * 0.99) & prior_20d.ge(-0.08) & prior_20d.le(0.30)

        pullback_depth = close.shift(1).divide(high.shift(1).rolling(self.pullback_lookback, min_periods=4).max()).sub(1.0)
        pullback_ok = pullback_depth.le(-0.012) & pullback_depth.ge(-0.12)
        support_ok = low.shift(1).rolling(3, min_periods=2).min().gt(sma60.shift(1) * 0.945)
        ...
        breadth_regime_ok = market_breadth.shift(1).ge(self.breadth_threshold) & breadth_ma5.shift(1).ge(0.47)
        breadth_regime_frame = pd.DataFrame(
            {col: breadth_regime_ok for col in close.columns},
            index=close.index,
        ).fillna(False)
        ...
        medium_trend_ok = close.ge(sma120 * 0.98)
        ...
        breadth_exit = market_breadth.lt(0.40) | market_breadth.diff(3).lt(-0.12)
        breadth_exit_frame = pd.DataFrame(
            {col: breadth_exit for col in close.columns},
            index=close.index,
        ).fillna(False)
        eligible_exit = (
            op_q1_rev.le(-0.012)
