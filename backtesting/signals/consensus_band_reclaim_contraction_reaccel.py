...
class ConsensusBandReclaimContractionReaccelSignalProducer:
    revision_lookback: int = 20
    trend_lookback: int = 60
    pullback_lookback: int = 7
    max_hold_days: int = 9
    revision_threshold: float = 0.0115
    volume_threshold: float = 0.84
    reclaim_buffer: float = -0.0012
    breadth_threshold: float = 0.37
    ...
    def build(self, market: MarketData) -> SignalBundle:
        ...
        trend_ok = close.gt(sma60 * 0.973) & sma20.gt(sma60 * 0.989) & prior_20d.ge(-0.08) & prior_20d.le(0.32)

        pullback_depth = close.shift(1).divide(high.shift(1).rolling(self.pullback_lookback, min_periods=4).max()).sub(1.0)
        pullback_ok = pullback_depth.le(-0.012) & pullback_depth.ge(-0.13)
        ...
        contraction_ok = contraction_ratio.le(0.84)
        ...
        breadth_regime_ok = (
            ...
            & breadth_ma5.shift(1).ge(0.39)
            & breadth_slope3.shift(1).ge(0.0)
        ) | (
            market_breadth.shift(1).ge(0.40)
            & breadth_ma5.shift(1).ge(0.405)
        ...
        close_strength = close.ge(open_ * 0.999) & close.ge((high + low) / 2.0) & close.ge(sma5 * 0.997)
        ...
        trailing_support = pd.concat([
            sma10 * 0.986,
            sma20 * 0.981,
            low.shift(1).rolling(4, min_periods=2).min() * 0.985,
        ], axis=0).groupby(level=0).max()
        breadth_exit = market_breadth.lt(0.355) | market_breadth.diff(3).lt(-0.08)
        ...
        alpha = (
            ...
            + 0.05 * close.divide(sma120.replace(0.0, pd.NA)).sub(1.0)
