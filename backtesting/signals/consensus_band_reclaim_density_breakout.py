...
class ConsensusBandReclaimDensityBreakoutSignalProducer:
    ...
    def build(self, market: MarketData) -> SignalBundle:
        ...
        trend_ok = close.gt(sma60 * 0.972) & sma20.gt(sma60 * 0.988) & prior_20d.ge(-0.09) & prior_20d.le(0.30)

        pullback_depth = close.shift(1).divide(high.shift(1).rolling(self.pullback_lookback, min_periods=4).max()).sub(1.0)
        pullback_ok = pullback_depth.le(-0.010) & pullback_depth.ge(-0.125)
        ...
        breadth_regime_ok = (
            ...
            & breadth_ma5.shift(1).ge(0.412)
            & breadth_slope3.shift(1).ge(-0.01)
        ) | (
            market_breadth.shift(1).ge(0.385)
            & breadth_slope3.shift(1).ge(0.02)
            & breadth_ma5.shift(1).ge(0.395)
        )
        ...
        breakout_reclaim = close.gt(reclaim_ref * (1.0 + self.reclaim_buffer)) & open_.le(reclaim_ref * 1.018)
        close_strength = close.ge(open_ * 1.001) & close.ge((high + low) / 2.0) & close.ge(sma5 * 0.999)
        medium_trend_ok = close.ge(sma120 * 0.968)
        trigger = breakout_reclaim & close_strength & volume_ratio.gt(self.volume_threshold) & contraction_ratio.le(0.96)
        ...
        breadth_exit = market_breadth.lt(0.392) | market_breadth.diff(3).lt(-0.09)
