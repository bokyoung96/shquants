...
class ConsensusBandReclaimDensityGuardSignalProducer:
    revision_lookback: int = 20
    trend_lookback: int = 60
    pullback_lookback: int = 7
    max_hold_days: int = 7
    revision_threshold: float = 0.011
    volume_threshold: float = 0.82
    reclaim_buffer: float = -0.002
    breadth_threshold: float = 0.405
    ...
    def build(self, market: MarketData) -> SignalBundle:
        ...
        pullback_ok = pullback_depth.le(-0.009) & pullback_depth.ge(-0.125)
        ...
        breadth_regime_ok = (
            ...
            & (
                breadth_ma5.shift(1).ge(0.42)
                | breadth_slope3.shift(1).ge(0.008)
            )
        )
        ...
        breakout_reclaim = close.gt(reclaim_ref * (1.0 + self.reclaim_buffer)) & open_.le(reclaim_ref * 1.02)
        bounce_reclaim = (
            low.le(sma10 * 1.01)
            & close.gt(sma5 * 1.001)
            & close.gt(sma20 * 0.995)
            & close.gt(open_ * 1.002)
            & close.shift(1).le(sma5.shift(1) * 1.01)
            & sma5.gt(sma10 * 0.998)
        )
        close_strength = close.ge(open_ * 0.998) & close.ge((high + low) / 2.0)
        medium_trend_ok = close.ge(sma120 * 0.97)
        trigger_a = breakout_reclaim & close_strength & volume_ratio.gt(self.volume_threshold) & contraction_ratio.le(0.95)
        trigger_b = bounce_reclaim & close_strength & volume_ratio.gt(0.78) & close.divide(sma10.replace(0.0, pd.NA)).sub(1.0).le(0.035)
        ...
        trailing_support = pd.concat([
            ...
            sma10 * 0.988,
            sma20 * 0.982,
            low.shift(1).rolling(3, min_periods=2).min() * 0.987,
        ], axis=0).groupby(level=0).max()
        breadth_exit = market_breadth.lt(0.40) | market_breadth.diff(3).lt(-0.08)
        ...
        alpha = (
            ...
            + 0.05 * close.divide(sma20.replace(0.0, pd.NA)).sub(1.0)
            + 0.05 * contraction_ratio.rsub(1.0)
            + 0.06 * pd.DataFrame({col: market_breadth for col in close.columns}, index=close.index)
            + 0.06 * close.divide(sma120.replace(0.0, pd.NA)).sub(1.0)
