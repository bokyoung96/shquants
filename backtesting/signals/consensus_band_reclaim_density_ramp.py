...
class ConsensusBandReclaimDensityRampSignalProducer:
    ...
    def build(self, market: MarketData) -> SignalBundle:
        ...
        trend_ok = close.gt(sma60 * 0.968) & sma20.gt(sma60 * 0.985) & prior_20d.ge(-0.10) & prior_20d.le(0.35)

        pullback_depth = close.shift(1).divide(high.shift(1).rolling(self.pullback_lookback, min_periods=4).max()).sub(1.0)
        pullback_ok = pullback_depth.le(-0.008) & pullback_depth.ge(-0.135)
        support_ok = low.shift(1).rolling(3, min_periods=2).min().gt(sma60.shift(1) * 0.935)
        ...
        breadth_slope3 = market_breadth.diff(3)
        breadth_regime_ok = (
            market_breadth.shift(1).ge(self.breadth_threshold)
            & (
                breadth_ma5.shift(1).ge(0.425)
                | breadth_slope3.shift(1).ge(0.01)
            )
        )
        ...
        breakout_reclaim = close.gt(reclaim_ref * (1.0 + self.reclaim_buffer)) & open_.le(reclaim_ref * 1.022)
        bounce_reclaim = (
            low.le(sma10 * 1.012)
            & close.gt(sma5 * 0.998)
            & close.gt(open_ * 1.001)
            & close.shift(1).le(sma5.shift(1) * 1.012)
        )
        close_strength = close.ge(open_ * 0.996) & close.ge((high + low) / 2.0)
        medium_trend_ok = close.ge(sma120 * 0.965)
        trigger_a = breakout_reclaim & close_strength & volume_ratio.gt(self.volume_threshold) & contraction_ratio.le(0.92)
        trigger_b = bounce_reclaim & close_strength & volume_ratio.gt(0.72)
        ...
        trailing_support = pd.concat([
            sma10 * 0.984,
            sma20 * 0.979,
            low.shift(1).rolling(4, min_periods=2).min() * 0.983,
        ], axis=0).groupby(level=0).max()
        breadth_exit = market_breadth.lt(0.385) | market_breadth.diff(3).lt(-0.12)
        ...
        alpha = (
            0.36 * op_q1_rev
            + 0.14 * op_y1_rev
            + 0.11 * close.divide(reclaim_ref.replace(0.0, pd.NA)).sub(1.0)
            - 0.05 * pullback_depth.abs()
            + 0.07 * volume_ratio
            + 0.06 * close.divide(sma10.replace(0.0, pd.NA)).sub(1.0)
            + 0.06 * close.divide(sma20.replace(0.0, pd.NA)).sub(1.0)
            + 0.06 * contraction_ratio.rsub(1.0)
            + 0.07 * pd.DataFrame({col: market_breadth for col in close.columns}, index=close.index)
            + 0.07 * close.divide(sma120.replace(0.0, pd.NA)).sub(1.0)
            + 0.10 * trigger_b.astype(float)
