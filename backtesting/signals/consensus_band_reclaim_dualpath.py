...
class ConsensusBandReclaimDualPathSignalProducer:
    revision_lookback: int = 20
    trend_lookback: int = 60
    pullback_lookback: int = 7
    max_hold_days: int = 9
    revision_threshold: float = 0.013
    ...
    def build(self, market: MarketData) -> SignalBundle:
        ...
        pullback_ok = pullback_depth.le(-0.01) & pullback_depth.ge(-0.12)
        ...
        close_strength = close.ge((high + low) / 2.0)
        trigger_a = breakout_reclaim & close_strength & volume_ratio.gt(self.volume_threshold) & contraction_ratio.le(0.88)
        trigger_b = bounce_reclaim & close_strength & volume_ratio.gt(0.80)
        ...
        alpha = (
            ...
            + 0.09 * close.divide(sma20.replace(0.0, pd.NA)).sub(1.0)
