...
class ConsensusBandReclaimTightExitSignalProducer:
    ...
    def build(self, market: MarketData) -> SignalBundle:
        ...
        revision_gate = op_q1_rev.gt(self.revision_threshold) & op_y1_rev.gt(-0.01)

        prior_20d = close.shift(1).divide(close.shift(21)).sub(1.0)
        trend_ok = close.gt(sma60 * 0.97) & sma20.gt(sma60 * 0.985) & prior_20d.ge(-0.08) & prior_20d.le(0.35)

        pullback_depth = close.shift(1).divide(high.shift(1).rolling(self.pullback_lookback, min_periods=4).max()).sub(1.0)
        pullback_ok = pullback_depth.le(-0.01) & pullback_depth.ge(-0.15)
        support_ok = low.shift(1).rolling(3, min_periods=2).min().gt(sma60.shift(1) * 0.93)
        ...
        close_strength = close.ge(open_ * 0.995) & close.ge((high + low) / 2.0)
        ...
        eligible_exit = (
            ...
            | close.gt(high.shift(1).rolling(10, min_periods=5).max() * 1.04)
            | close.isna()
        )
        ...
        alpha = (
            ...
            + 0.10 * volume_ratio
            + 0.10 * close.divide(sma10.replace(0.0, pd.NA)).sub(1.0)
