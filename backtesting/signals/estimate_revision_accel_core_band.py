...
class EstimateRevisionAccelCoreBandSignalProducer:
    ...
    def build(self, market: MarketData) -> SignalBundle:
        ...
        eps_q1_rev = eps_fwd_q1.pct_change(fill_method=None)
        op_fy1_rev = op_fwd.pct_change(fill_method=None)
        revision_raw = 0.45 * op_q1_rev + 0.35 * eps_q1_rev + 0.20 * op_fy1_rev
        price_momentum = close.pct_change(self.momentum_lookback, fill_method=None)

        revision_score = self._cross_sectional_zscore(revision_raw)
        momentum_score = self._cross_sectional_zscore(price_momentum)
        alpha = revision_score + 0.2 * momentum_score

        event_now = (
            revision_raw.gt(self.revision_threshold)
            & op_q1_rev.gt(0.0)
            & eps_q1_rev.gt(0.0)
            & price_momentum.gt(-0.05)
            & close.notna()
        )
        event_prev = event_now.shift(1).fillna(False)
        eligible_entry = event_now & ~event_prev
        eligible_add_1 = event_now & event_prev
        eligible_add_2 = eligible_add_1 & price_momentum.gt(0.0)
