...
class ConsensusBetaPersistenceConcentratedLongOnly(ComposableStrategy):
    lookback: int = 60
    ...
    def __post_init__(self) -> None:
        self.signal_producer = _ConsensusBetaPersistenceSignalProducer(
            lookback=self.lookback,
            flow_lookback=self.flow_lookback,
            beta_lookback=max(120, self.momentum_lookback),
            top_n=self.top_n,
        )
        self.construction_rule = _ConcentratedLongOnlyConstruction(top_n=self.top_n)


@dataclass(slots=True)
class _ConsensusBetaPersistenceSignalProducer:
    lookback: int = 60
    flow_lookback: int = 20
    ...
    def build(self, market: MarketData) -> SignalBundle:
        ...
        breadth_persistence = market_breadth.rolling(2, min_periods=2).min().fillna(-1.0)

        gross_long = pd.Series(0.0, index=close.index, dtype=float)
        ...
        for timestamp in close.index:
            ...
            if len(high_beta_names) < max(4, self.top_n):
                continue

            candidate_pool = high_beta_names.intersection(pos_row[pos_row].index)
            if len(candidate_pool) < max(4, self.top_n // 2):
                continue

            leadership_breadth = float(pos_row.reindex(high_beta_names).fillna(False).mean())
            risk_on = (
                market_breadth.loc[timestamp] > breadth_mean.loc[timestamp]
                and breadth_delta.loc[timestamp] > 0.0
                and breadth_persistence.loc[timestamp] > 0.0
                and leadership_breadth >= 0.55
                and benchmark_trend.loc[timestamp] > 0.0
            )
            if not risk_on:
                continue

            strong_confirmation = leadership_breadth >= 0.65 and breadth_delta.loc[timestamp] > breadth_delta.rolling(self.lookback, min_periods=max(20, self.lookback // 3)).mean().fillna(0.0).loc[timestamp]
            gross_long.loc[timestamp] = 1.0 if strong_confirmation else 0.75
...
@dataclass(slots=True)
class _ConcentratedLongOnlyConstruction:
    top_n: int = 10
    ...
    def build(self, bundle: SignalBundle) -> ConstructionResult:
        ...
        if not isinstance(sector, pd.DataFrame) or not isinstance(selected, pd.DataFrame):
            raise ValueError("concentrated long-only construction requires sector and selected context")
        ...
        if isinstance(gross_long, pd.Series):
            gross_long = gross_long.reindex(alpha.index).fillna(0.0).astype(float)
        else:
            gross_long = pd.Series(0.0, index=alpha.index, dtype=float)
