...
class EarningsRevision(ComposableStrategy):
    top_n: int = 30
    lookback: int = 20
    pth: float = 0.6

    def __post_init__(self) -> None:
        self.signal_producer = _EarningsRevisionSignal(
...
class _EarningsRevisionSignal:
    ...
    def build(self, market: MarketData) -> SignalBundle:
        ...
        op_pos = op_rev.gt(0)
        participation = 0.5 * eps_pos.astype(float) + 0.5 * op_pos.astype(float)

        eps_rank = eps_rev.rank(axis=1, pct=True)
        op_rank = op_rev.rank(axis=1, pct=True)
        alpha = 0.5 * eps_rank + 0.5 * op_rank
        alpha = alpha.where(eps_pos & op_pos & participation.ge(self.pth))

        return SignalBundle(
            alpha=alpha,
            context={"tradable": alpha.notna()},
            meta={},
        )
