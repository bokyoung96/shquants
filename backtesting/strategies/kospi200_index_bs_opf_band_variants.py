...
from backtesting.signals.kospi200_index_bs_opf_band import Kospi200IndexBsOpfBandSignalProducer

from .base import RegisteredStrategy


@dataclass(slots=True)
class _Kospi200IndexBsOpfBandBase(RegisteredStrategy):
    variant: str = 'balanced'

    def __post_init__(self) -> None:
        self.signal_producer = Kospi200IndexBsOpfBandSignalProducer()
    ...
    def build_plan(self, market) -> PositionPlan:
        ...
        weights = pd.DataFrame({'IKS200': exposure}, index=band_state.index).where(tradable, 0.0).fillna(0.0)
...
@dataclass(slots=True)
class Kospi200IndexBsOnlyBand(_Kospi200IndexBsOpfBandBase):
    variant: str = 'bs_only'


@dataclass(slots=True)
class Kospi200IndexBsOpfBandBalanced(_Kospi200IndexBsOpfBandBase):
    variant: str = 'balanced'


@dataclass(slots=True)
class Kospi200IndexBsOpfBandStrict(_Kospi200IndexBsOpfBandBase):
    variant: str = 'strict'


@dataclass(slots=True)
class Kospi200IndexBsOpfBandAggressive(_Kospi200IndexBsOpfBandBase):
