...
class RevisionOiHighBetaMomentumLs(ComposableStrategy):
    lookback: int = 20
    flow_lookback: int = 20
    momentum_lookback: int = 60

    def __post_init__(self) -> None:
        self.signal_producer = _RevisionOiHighBetaMomentumSignalProducer(
...
@dataclass(slots=True)
class _RevisionOiHighBetaMomentumSignalProducer:
    lookback: int = 20
    flow_lookback: int = 20
    momentum_lookback: int = 60
    ...
    def build(self, market: MarketData) -> SignalBundle:
        ...
        for timestamp in eps.index:
            ...
            for _, members in sector_row.groupby(sector_row, sort=False):
                ...
                long_combo = 0.65 * beta_momo_z + 0.35 * revision_z
                long_cut = long_combo.quantile(0.6)
                longs = long_combo[long_combo >= long_cut].index
                longs = longs[long_frame.reindex(longs)["momentum"].fillna(0.0).gt(0.0).to_numpy()]
                longs = longs[long_frame.reindex(longs)["beta"].fillna(0.0).gt(0.8).to_numpy()]
