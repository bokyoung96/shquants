...
class RevisionOiSoftBetaTiltMomentumLs(ComposableStrategy):
    lookback: int = 20
    flow_lookback: int = 20
    momentum_lookback: int = 60

    def __post_init__(self) -> None:
        self.signal_producer = _RevisionOiSoftBetaTiltMomentumSignalProducer(
...
@dataclass(slots=True)
class _RevisionOiSoftBetaTiltMomentumSignalProducer:
    lookback: int = 20
    flow_lookback: int = 20
    momentum_lookback: int = 60
    ...
    def build(self, market: MarketData) -> SignalBundle:
        ...
        beta = beta.reindex(trading_index)
        ...
        for timestamp in eps.index:
            ...
            beta_row = beta.loc[timestamp].reindex(score_row.index)
            ...
            for _, members in sector_row.groupby(sector_row, sort=False):
                ...
                long_frame = pd.concat(
                    [
                        ...
                        beta_row.reindex(long_candidates.index).rename("beta"),
                    ],
                    axis=1,
                ).dropna()
                if len(long_frame) < 3:
                ...
                beta_momo_z = self._zscore(long_frame["beta_momo"])
                revision_z = self._zscore(long_frame["revision_score"])
                momentum_z = self._zscore(long_frame["momentum"])
                beta_z = self._zscore(long_frame["beta"])
                long_combo = 0.45 * revision_z + 0.30 * beta_momo_z + 0.15 * momentum_z + 0.10 * beta_z
                long_cut = long_combo.quantile(0.55)
                longs = long_combo[long_combo >= long_cut].index
                longs = longs[long_frame.reindex(longs)["momentum"].fillna(0.0).ge(0.0).to_numpy()]
                ...
                alpha.loc[timestamp, longs] = long_combo.reindex(longs).fillna(0.0).astype(float)
