...
class RevisionOiStateConditionedShortSqueezeBetaCapLs(ComposableStrategy):
    lookback: int = 20
    flow_lookback: int = 20
    momentum_lookback: int = 60

    def __post_init__(self) -> None:
        self.signal_producer = _RevisionOiStateConditionedShortSqueezeBetaCapSignalProducer(
...
@dataclass(slots=True)
class _RevisionOiStateConditionedShortSqueezeBetaCapSignalProducer:
    lookback: int = 20
    flow_lookback: int = 20
    momentum_lookback: int = 60
    ...
    def build(self, market: MarketData) -> SignalBundle:
        ...
        for timestamp in eps.index:
            ...
            for _, members in sector_row.groupby(sector_row, sort=False):
                ...
                if use_overlay:
                    filtered_guard = short_guard.reindex(filtered)
                    beta_cut = filtered_guard["beta_momo"].quantile(0.7)
                    squeeze_chasers = filtered_guard.index[
                        filtered_guard["momentum"].gt(0.0) & filtered_guard["beta_momo"].ge(beta_cut)
                    ]
                    if len(squeeze_chasers) > 0:
                        conf.loc[squeeze_chasers] = conf.reindex(squeeze_chasers).fillna(0.0) * 0.35
