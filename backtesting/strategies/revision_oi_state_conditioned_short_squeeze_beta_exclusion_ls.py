...
class RevisionOiStateConditionedShortSqueezeBetaExclusionLs(ComposableStrategy):
    lookback: int = 20
    flow_lookback: int = 20
    momentum_lookback: int = 60

    def __post_init__(self) -> None:
        self.signal_producer = _RevisionOiStateConditionedShortSqueezeBetaExclusionSignalProducer(
...
@dataclass(slots=True)
class _RevisionOiStateConditionedShortSqueezeBetaExclusionSignalProducer:
    lookback: int = 20
    flow_lookback: int = 20
    momentum_lookback: int = 60
    ...
    def build(self, market: MarketData) -> SignalBundle:
        ...
        benchmark_trend = benchmark.divide(benchmark.rolling(beta_lookback, min_periods=max(40, beta_lookback // 3)).mean()) - 1.0
        ...
        benchmark_trend = benchmark_trend.reindex(trading_index)
        ...
        risk_on_state = market_balance.gt(market_mean) & benchmark_trend.gt(0.0)
        ...
        for timestamp in eps.index:
            ...
            use_overlay = bool(risk_on_state.loc[timestamp])
            ...
            for _, members in sector_row.groupby(sector_row, sort=False):
                ...
                short_guard = pd.concat(
                    [
                        ...
                        beta_momo_row.reindex(raw_shorts).rename("beta_momo"),
                        momentum_row.reindex(raw_shorts).rename("momentum"),
                    ],
                    axis=1,
                ).dropna()
                ...
                guarded = short_guard
                if use_overlay:
                    beta_cut = short_guard["beta_momo"].quantile(0.7)
                    squeeze_chasers = short_guard.index[
                        short_guard["momentum"].gt(0.0) & short_guard["beta_momo"].ge(beta_cut)
                    ]
                    guarded = short_guard.drop(index=squeeze_chasers, errors="ignore")
                    if len(guarded) < 2:
                        guarded = short_guard[~short_guard["momentum"].gt(0.0)]
                    if len(guarded) < 2:
                        guarded = short_guard

                oi_level_z = self._zscore(guarded["oi_level"])
                oi_imp_z = self._zscore(guarded["oi_impulse"])
                revision_short_z = self._zscore(guarded["revision_score"])
                squeeze_risk = 0.75 * oi_level_z + 0.75 * oi_imp_z + 0.25 * revision_short_z
                keep_mask = squeeze_risk <= 0.25
                filtered = guarded.index[keep_mask.reindex(guarded.index).fillna(False)]
                if len(filtered) == 0:
                    continue
