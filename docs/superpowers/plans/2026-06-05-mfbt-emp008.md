...
**Goal:** Build an `emp008`-internal MFBT Barra pipeline that turns five MFBT raw factors into rolling KOSPI200 benchmark-relative target weights.

**Architecture:** Keep `backtesting/strategies/mfbt.py` unchanged. Implement the new pipeline inside `backtesting/strategies/emp008/` using shquants catalog/parquet data, continuous raw exposures, float-market-cap preprocessing, explicit sector exposure names, rolling regression/risk estimation, and constrained active-weight optimization against `QW_BM_WEIGHTS`. Remove or bypass legacy `emp008` data paths once the new path has equivalent tested behavior.
