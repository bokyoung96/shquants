...
df['alpha_overlay'] = 0.01 * df['exposure']
periods = [
...
for ax, (start, end, title) in zip(axes, periods):
    ...
    ax.plot(sub.index, ov, color='#d62728', linewidth=2.1, label='BM 100% + max 1% overlay')
    ax.set_title(f'{title}, Equity with Alpha Overlay')
    ax.set_ylabel('Normalized Equity')
    ax.grid(True, alpha=0.25)

    ax2 = ax.twinx()
    ax2.fill_between(sub.index, 0, sub['alpha_overlay'], color='#1f77b4', alpha=0.18, label='Alpha overlay')
    ax2.plot(sub.index, sub['alpha_overlay'], color='#1f77b4', linewidth=1.4)
    ax2.set_ylabel('Alpha overlay')
    ax2.set_ylim(0, max(0.011, sub['alpha_overlay'].max() * 1.2 if len(sub) else 0.011))
...
plt.tight_layout()
out = base / 'quantile_breakout_overlay_alpha_subplots.png'
