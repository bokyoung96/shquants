...
from backtesting.reporting.models import ReportKind, ReportProfile
...
def test_cli_builds_report_spec_with_auto_kind_and_benchmark(tmp_path: Path, monkeypatch) -> None:
    ...
    assert captured["spec"].profile is None
...
def test_cli_supports_profile_override_and_no_benchmark(tmp_path: Path, monkeypatch) -> None:
    cli = ReportCli(runs_root=tmp_path, reports_root=tmp_path / "reports")
    ...
    assert captured["spec"].kind is ReportKind.TEARSHEET
    assert captured["spec"].profile is ReportProfile.ABSOLUTE
    assert captured["spec"].benchmark is None
