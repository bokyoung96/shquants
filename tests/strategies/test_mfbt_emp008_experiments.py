...
def test_factor_contribution_panel_uses_shared_prepared_bundle(monkeypatch: pytest.MonkeyPatch) -> None:
    config = MfbtEmp008Config(factor_set="mfbt_origin_smallcap", risk_window=1)
    ...
    monkeypatch.setattr(
        "backtesting.strategies.emp008.mfbt_emp008_experiments.active_weight_factor_plots.load_mfbt_emp008_market",
        fail,
    )
    monkeypatch.setattr(
        "backtesting.strategies.emp008.mfbt_emp008_experiments.active_weight_factor_plots.build_raw_mfbt_factors",
        fail,
    )
    monkeypatch.setattr(
        "backtesting.strategies.emp008.mfbt_emp008_experiments.active_weight_factor_plots.preprocess_factor_frame",
        fail,
    )
    monkeypatch.setattr(
        "backtesting.strategies.emp008.mfbt_emp008_experiments.active_weight_factor_plots.build_sector_active_exposures",
        fail,
    )
    monkeypatch.setattr(
        "backtesting.strategies.emp008.mfbt_emp008_experiments.active_weight_factor_plots._neutralize_large_benchmark_weight_factor_exposures",
        fail,
    )
    monkeypatch.setattr(
        "backtesting.strategies.emp008.mfbt_emp008_experiments.active_weight_factor_plots.load_and_prepare_emp008_factors",
        lambda parquet_dir, start, end, config: prepared,
    )
    monkeypatch.setattr(
        "backtesting.strategies.emp008.mfbt_emp008_experiments.active_weight_factor_plots.build_strategy_config",
        lambda strategy, tracking_error_annual, risk_model: config,
    )
    monkeypatch.setattr(
        "backtesting.strategies.emp008.mfbt_emp008_experiments.active_weight_factor_plots.fit_cross_sectional_factor_returns",
    ...
    monkeypatch.setattr(
        "backtesting.strategies.emp008.mfbt_emp008_experiments.active_weight_factor_plots.compute_expected_alpha",
